import type { Sender, ResponseEnvelope, PaginationMeta } from './types';
import { defaultConfig, type ResponderConfig } from './config';
...
export class BaseResponder<P = unknown, M = PaginationMeta> {
    ...
    protected normalizeError(err: unknown): { message: string; code?: string; details?: unknown } {
        if (err instanceof Error) return { message: err.message, code: (err as any).code };
        if (typeof err === 'string') return { message: err };
        return { message: 'Unknown error', details: err };
    }

    protected buildEnvelope(data?: P, message?: string, error?: unknown, meta?: M) {
