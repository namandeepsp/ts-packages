...
export class CryptoManager {
    ...
    public deriveKey(
        ...
        return new Promise((resolve, reject) => {
            ...
            crypto.pbkdf2(password, salt, iterations, keyLength, 'sha256', (err: Error, derivedKey: Buffer) => {
                if (err) {
                    reject(err);
                } else {
     ...
     */
    public rsaDecrypt(encryptedData: string, privateKey: string): Promise<string> {
        return new Promise((resolve, reject) => {
            const crypto = require('crypto');
     ...
     */
    public rsaSign(data: string, privateKey: string, algorithm: string = 'sha256'): Promise<string> {
        return new Promise((resolve, reject) => {
            const crypto = require('crypto');
 ...
 */
export const createCryptoManager = (config?: CryptoManagerConfig): CryptoManager => {
